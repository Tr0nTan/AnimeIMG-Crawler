...
def geturls(names):
    ...
    while True:
        ...
        for url in re.findall('"file_url":"(.*?)"',tags): # iterate over all images' url in one page
            ...
            name_id = [re.search('\d+', name).group(0) for name in names]
...
if __name__=='__main__':
    ...
    if os.path.exists('Yande\\all_names.txt'):
        f = open('Yande\\all_names.txt','r', encoding="utf-8")
    ...
    f = open('Yande\\all_names.txt','w', encoding= 'utf-8')
    for name in names:
        f.write(name+'\n')
    f.close()
